                                ...
                                <select name="angkatan" class="form-select" onchange="this.form.submit()">
                                    ...
                                    @foreach ($angkatanList as $item)
                                        <option value="{{ $item->angkatan }}"
                                            {{ request('angkatan') == $item->angkatan ? 'selected' : '' }}>
                                            {{ $item->angkatan }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="card-body">
                            @if (request('angkatan'))
                                ...
                                @if ($matkulByAngkatan->isNotEmpty())
                                                    ...
                                                    @foreach ($matkulAngkatan as $index => $m)
                                                        <tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                <form
                                                                    action="{{ route('admin.matkul.destroy', $m->id_matkul) }}"
                                    ...
                                    @endforeach
                                @else
                                    <p class="text-center mt-3">Tidak ada data mata kuliah untuk angkatan ini.</p>
                                @endif
                            @else
                                <div class="alert alert-info text-center">
                                    Silakan pilih angkatan terlebih dahulu untuk melihat data mata kuliah.
                                </div>
                            @endif
                        </div>
                    </div>
